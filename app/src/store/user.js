...
const api = {
  ...
  logout: async function () {
    localStorage.removeItem("ops/testMode");
  ...
  switchWorkspace: async function (form) {
    try {
      const res = await http.post(`/user/switch-workspace/${form.newWorkspace}`);
      return res.data || [];
    } catch (err) {
      console.log(err);
      throw err;
  ...
  actions: {
    ...
    signup: async function (form = {}, pie = null) {
      ...
      if (pie) {
        ...
        app.sendNotification({
          type: "success",
          message: `Welcome to Operational, ${this.resource.firstName}`,
        });
        app.startOnboarding();
      } else {
        return false;
