...
description: Discover how to track cart events in your SaaS product using Operational for better user insights.
date: 2024-06-11
...
Operational is an open source event tracking tool for any product. It uses a single API to log events from your SaaS. You can use it to log cart events and view them in real time.
...
await ops.events.log(addItemPayload);
...
await ops.events.log(removeItemPayload);
...
await ops.events.log(checkoutPayload);
```
...
Learn more at https://operational.co. Try the [playground](https://operational.co/playground) to copy and paste snippets.
