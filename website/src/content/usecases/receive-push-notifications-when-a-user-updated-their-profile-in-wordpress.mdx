...
description: Learn to set up push notifications for user profile updates in Wordpress using Operational's event tracking API.
date: 2024-06-12
tags:
  - wordpress
  - notifications
...
This guide shows how to send those alerts automatically with Operational.

![Operational in action](https://operational.co/images/pitch/ops_insitu.jpg)

## What is Operational
Operational is an open source event tracking service. You can send any user action to it and get instant alerts.
...
It builds an event payload and sends it to Operational’s ingest API.
...
add_action('profile_update', function($userId, $oldData) {
    ...
    $response = wp_remote_post('https://api.operational.co/v1/events.ingest', [
      'headers' => [
...
This snippet listens for profile changes and sends a push notification event to Operational.

## Conclusion
With Operational you avoid building custom notification code.
You get real-time alerts for profile updates in minutes.
Learn more on the [Operational homepage](https://operational.co) or try code samples in the [playground](https://operational.co/playground).