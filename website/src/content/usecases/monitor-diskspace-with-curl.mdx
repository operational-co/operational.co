...
Operational provides a simple API to log events from any tool or script.
...
```bash
curl -X POST https://api.operational.co/api/v1/log -H "Authorization: Bearer YOUR_API_KEY" -H "Content-Type: application/json" --data '{
  "name": "Low disk space",
  "avatar": "💽",
