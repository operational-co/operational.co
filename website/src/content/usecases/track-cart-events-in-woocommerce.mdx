...
description: Capture add to cart actions in WooCommerce and send them to Operational for real-time insights.
date: 2024-06-10
...
Operational is an open-source service for capturing events in any application.
...
add_action('woocommerce_add_to_cart', function($cart_item_key, $product_id, $quantity) {
    ...
    $client->post('https://api.operational.co/api/v1/ingest', [
        'headers' => [
...
Then it sends the event to Operational using an HTTP POST request.

### Conclusion

Operational saves you time by handling event ingestion and analytics.
