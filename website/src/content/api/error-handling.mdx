...
slug: error-handling
description: Understand how errors are handled in Operational's event api
tags: ["API"]
date: 2024-06-08
---
...
<ApiItem client:load title="400" type="event wasn't created" required="">
  Your event payload was deformed, or badly formatted. See the [log endpoint docs](/api/log-event) for info.
</ApiItem>
...
This means there's a very likely possibility you might send a request that won't make it past our parsers. Don't worry, we try to log the event data whenever possible.
...
- Events cannot be more than 100kb big. If they are, we will try to figure out if the event's type was json and if it was, it will be truncated. If the truncated event is under 100kb, ops will log the event.

In these scenarios, we'll add warnings under the event letting you know that the event was modified so it could be logged.
