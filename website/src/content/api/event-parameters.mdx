...
<ApiItem client:load title="category" type="string" required="false">
  Use this to group common events by a specific category, eg: error, user, billing, etc. Read more about categories [here.](/manual/categories-and-search)
</ApiItem>
...
<ApiItem client:load title="contextId" type="string" required="false">
  Defaults to null. Related to [contexts.](/docs/group-events-with-contexts)
</ApiItem>
<ApiItem client:load title="contextStart" type="boolean" required="false">
  Defaults to false. Related to [contexts.](/docs/group-events-with-contexts)
</ApiItem>
